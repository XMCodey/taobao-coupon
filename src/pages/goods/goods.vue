<template>
  ...
  <swiper class="swiper" indicator-dots="true">
    <swiper-item v-for="img in goodsData.small_images.string" class="swiper__item" :key='img'>
      <image mode="widthFix" class="swiper__image" :src="img"></image>
    </swiper-item>
  </swiper>
...
export default {
  ...
  onPageScroll(object) {
    if (object.scrollTop > 100) {
      this.changeTopMenuCss = true
    } else if (object.scrollTop > 500) {
    } else {
      this.changeTopMenuCss = false
    }
    // 220是375px宽度下到元素高度
    if (object.scrollTop > (this.windowWidth / 750) * 220 * 2) {
